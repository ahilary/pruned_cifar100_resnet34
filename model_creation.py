...
def initial_model(train_loader, model_path, name):
    ...
    if name == 'resnet_34':
        ...
        fourth_init_model = fourth_level.fourth_level_pruning()
        fourth_init_model.fc = nn.Linear(fourth_init_model.fc.in_features, num_classes)
        fourth_init_model.to(device)
                        
        
        
        optimizer = optim.SGD(fourth_init_model.parameters(), lr=cnf_lr, momentum=0.9, weight_decay=5e-4)
    ...
    # model training happens for selecting best channels
        resnet_34_pruning_4.train_for_channel_wgts(train_loader, fourth_init_model, optimizer, criterion, model_path)
...
def pruned_model(model_path, name, best_wgt_epoch):
    ...
    if name == 'resnet_34':
        ...
        prune_obj = resnet_34_pruning_4.create_prune_model(model_path, best_wgt_epoch) 
        prune_obj.pruning()
